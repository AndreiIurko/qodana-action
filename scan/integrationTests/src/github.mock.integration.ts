...
export const cacheRestoreSpy = sinon.spy(cache, 'restoreCache')
...
export function setupGithubCoreMock(
  mockSummaryWrite: sinon.SinonStub | undefined,
  setFailed: sinon.SinonStub<[message: string | Error], void> | undefined
) {
  if (mockSummaryWrite != undefined) {
  ...
  if (setFailed != undefined) {
    sinon.stub(core, 'setFailed').callsFake(setFailed)
  }
}
...
export function setupGithubMock(
  contextToUse: Context.Context = getDefaultGithubContext(),
  issues: IssuesRest = {
